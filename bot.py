...
def handle_callback_query(update):
    ...
    if data == 'puzzle':
        send_message(chat_id, "🧩 Puzzle Game Starting!\nHere's your puzzle:\nWhat has keys but can't open locks?")
    elif data == 'word':
        send_message(chat_id, "✍️ Word Game Starting!\nUnscramble this word: 'LPAIN'")
...
def ask_gemini(prompt, sender_name="User"):
...
def process_message(message):
    ...
    # Handle /gameon command
    if text_lower == "/gameon":
        handle_gameon_command(chat_id)
        return
...
def webhook():
    ...
    # Handle callback query first
    if "callback_query" in update:
        handle_callback_query(update)
        return jsonify({"ok": True})
    # Otherwise, handle message as usual
